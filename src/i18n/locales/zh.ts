export default {
  nav: {
    writings: '文字',
    series: '系列',
    projects: '工程',
    tags: '标签',
    about: '自述',
    friends: '友链',
    login: '登录',
    logout: '退出',
    settings: '个人设置',
  },
  home: {
    title: '最新文章',
    noPosts: '暂无文章',
    description: '个人博客 - 记录技术与生活',
    pageTitle: "mrwuliu's blog",
  },
  pagination: {
    prev: '← 上一页',
    next: '下一页 →',
    pageInfo: (p: number, t: number) => `第 ${p} 页 / 共 ${t} 页`,
  },
  writings: {
    title: '文字',
    noPosts: '暂无文章',
    pageTitle: '文字 - Blog',
    description: '所有文章列表 - 技术与生活随笔',
  },
  projects: {
    title: '工程',
    noProjects: '暂无项目',
    detail: '详细 →',
    visit: '访问 →',
    pageTitle: '工程 - Blog',
    description: '个人开源项目与作品集',
  },
  projectDetail: {
    techStack: '技术栈',
    backToProjects: '← 返回工程列表',
    created: '创建日期',
    updated: '更新日期',
  },
  series: {
    title: '系列文章',
    subtitle: '按主题整理的系列文章合集',
    empty: '暂无系列',
    posts: '篇文章',
    backToSeries: '← 返回系列列表',
    collectionPosts: '本系列文章',
    lastUpdated: '最后更新',
    pageTitle: '系列文章 - Blog',
    description: '按主题整理的系列文章合集',
  },
  tagsCloud: {
    title: '标签',
    noTags: '暂无标签',
    pageTitle: '标签 - Blog',
    description: '所有文章标签分类',
  },
  tag: {
    allTags: '所有标签',
    pageTitle: (name: string) => `标签: ${name}`,
    pageDescription: (name: string) => `标签"${name}"下的文章`,
    noPosts: '该标签下暂无文章',
  },
  post: {
    toc: '目录',
    copyCode: '复制',
    copiedCode: '已复制',
    copyCodeFailed: '复制失败',
    comments: (count: number) => `评论 (${count})`,
    noComments: '暂无评论',
    leaveComment: '发表评论',
    nameLabel: '昵称 *',
    namePlaceholder: '昵称 *（默认 momo）',
    emailLabel: '邮箱（可选）',
    emailNote: '您的邮箱不会被公开。勾选下方选项可在收到回复时获得邮件提醒。',
    contentLabel: '评论内容 *',
    submit: '提交评论',
    commentSuccess: '评论已提交，需要人工审核后才会显示。',
    commentError: '评论提交失败，请稍后再试。',
    reply: '回复',
    replyTo: (name: string) => `回复 ${name}`,
    cancelReply: '取消回复',
    notifyOnReply: '收到回复时邮件提醒我',
    loggedInAs: (name: string) => `已登录：${name}`,
    loggingIn: '已登录，正在加载…',
  loginToComment: '登录后评论',
  loginToCommentDesc: '评论需要先登录。',
    orLeaveAnonymous: '或匿名评论',
    anonymousHint: '匿名评论需要审核后才会显示。',
    submitting: '提交中…',
  },
  login: {
    title: '登录',
    pageTitle: '登录 - Blog',
    description: '使用邮箱验证码登录',
    subtitle: '使用邮箱验证码登录，无需密码',
    emailLabel: '邮箱',
    emailPlaceholder: '请输入邮箱地址',
    sendCode: '发送验证码',
    sending: '发送中…',
    codeLabel: '验证码',
    codePlaceholder: '6 位验证码',
    codeSent: '验证码已发送至邮箱',
    codeSentDesc: '请检查收件箱（及垃圾邮件文件夹）。',
    verify: '验证并登录',
    verifying: '验证中…',
    resend: '重新发送',
    resendIn: (s: number) => `${s}s 后可重发`,
    invalidEmail: '邮箱格式不正确',
    invalidCode: '验证码错误或已过期',
    rateLimited: '操作过于频繁，请稍后再试',
    backHome: '← 返回首页',
    changeEmail: '更换邮箱',
    resendFailed: '重发失败，请重试',
    loginSuccess: '登录成功',
    redirecting: '正在跳转…',
    bannedUser: '该账号已被禁用',
    sendFailed: '验证码发送失败，请稍后再试',
    backToEmail: '← 返回',
  },
  settings: {
    title: '个人设置',
    avatar: '头像',
    avatarSource: '头像来源',
    randomAvatar: '随机头像',
    changeAvatar: '换一个',
    useGravatar: '使用邮箱头像',
    uploadAvatar: '上传头像',
    uploadHint: '支持 JPG / PNG / GIF / WebP，最大 2MB',
    fileTooLarge: '文件过大（上限 2MB）',
    invalidFile: '不支持的文件格式',
    profile: '账户',
    name: '昵称',
    editName: '修改昵称',
    nameSaved: '昵称已更新',
    email: '邮箱',
    replyNotify: '回复通知',
    replyNotifyDesc: '有人回复我的评论时，通过邮件通知我',
    saved: '已保存',
    error: '保存失败，请重试',
  },
  about: {
    title: '自述',
    pageTitle: '自述 - Blog',
    noContent: '暂无内容',
    description: '关于作者 - 技术与生活',
  },
  friends: {
    title: '友情链接',
    noLinks: '暂无友链',
    pageTitle: '友情链接 - Blog',
    visit: '访问 →',
    description: '推荐博客与友情链接',
  },
  notFound: {
    title: '404 — 页面未找到',
    message: '页面未找到',
    backHome: '← 返回首页',
    description: '页面未找到',
  },
  unsubscribe: {
    title: '取消订阅',
    success: '已取消订阅',
    successDesc: '您将不再收到回复提醒邮件。',
    invalid: '链接无效',
    invalidDesc: '退订链接已失效或过期。',
    backHome: '← 返回首页',
  },
  footer: {
    copyright: (year: number) => `© ${year} Blog. Powered by Cloudflare Workers.`,
  },
}
