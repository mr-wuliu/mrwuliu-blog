...
const Toc: FC<{ headings: TocHeading[] }> = ({ headings }) => {
  ...
  return (
    <nav class="my-8 p-6 bg-white border border-black rounded-none">
      <h2 class="text-xs font-bold uppercase tracking-widest opacity-50 mb-4">目录</h2>
      <ul class="space-y-1">
        {headings.map((h) => (
...
const CommentSection: FC<{ comments: Comment[]; postSlug: string }> = ({ comments, postSlug }) => {
  return (
    <section class="mt-4 pt-4 border-t border-black">
      <h2 class="text-xl font-bold tracking-tight mb-6">评论 ({comments.length})</h2>
          ...
          {comments.map((c) => (
            <div class="p-6 bg-white border border-black mb-4" id={`comment-${c.id}`}>
              <div class="flex items-center gap-3 mb-2">
                ...
                <span class="text-xs font-bold uppercase tracking-widest opacity-50">{formatDate(c.createdAt)}</span>
              </div>
      ...
      <form class="p-6 border border-black space-y-4" method="post" action={`/posts/${postSlug}/comments`}>
        <h3 class="text-lg font-bold tracking-tight mb-4">发表评论</h3>
        <div>
          <label for="authorName" class="block text-xs font-bold uppercase tracking-widest opacity-50 mb-2">昵称 *</label>
        ...
        <div>
          <label for="authorEmail" class="block text-xs font-bold uppercase tracking-widest opacity-50 mb-2">邮箱（可选）</label>
          <input type="email" id="authorEmail" name="authorEmail" maxlength={100}
            class="w-full px-4 py-3 border border-black text-sm focus:outline-none focus:border-black" />
        </div>
        <div>
          <label for="content" class="block text-xs font-bold uppercase tracking-widest opacity-50 mb-2">评论内容 *</label>
        ...
        <button type="submit"
          class="px-8 py-3 font-bold text-sm border border-black rounded-none uppercase tracking-widest hover:bg-black hover:text-white transition-all">
          提交评论
        </button>
      </form>
    </section>
...
const PostNav: FC<{ prev: PostNav | null; next: PostNav | null }> = ({ prev, next }) => {
  ...
  return (
    <nav class="mt-6 pt-4 border-t border-black flex justify-between">
      <div>
        {prev && (
          <a href={`/posts/${prev.slug}`} class="text-sm font-bold text-black opacity-70 hover:opacity-100 no-underline transition-all inline-flex items-center gap-1.5">
      ...
      <div>
        {next && (
          <a href={`/posts/${next.slug}`} class="text-sm font-bold text-black opacity-70 hover:opacity-100 no-underline transition-all inline-flex items-center gap-1.5">
            {next.title}
        ...
        <div class="mt-4 mb-8 flex items-center gap-4">
          ...
          {post.tags.length > 0 && (
            <ul class="flex flex-wrap gap-1.5 list-none p-0 m-0">
              {post.tags.map((t) => (
                <li class="m-0">
                  <a class="text-[10px] font-black uppercase tracking-widest border border-black border-opacity-50 px-2 py-0.5 text-black hover:bg-black hover:text-white transition-all no-underline" href={`/tags/${t.slug}`}>{t.name}</a>
                </li>
              ))}
            </ul>
          )}
